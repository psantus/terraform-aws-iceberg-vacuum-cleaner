# Step Functions State Machine
resource "aws_sfn_state_machine" "vacuum_iceberg_tables" {
  name     = var.stepfunction_name
  role_arn = aws_iam_role.vacuum_iceberg_tables_role.arn

  definition = jsonencode({
    Comment = "A workflow to vacuum Iceberg tables in a Glue database"
    StartAt = "GetGlueTables"
    States = {
      GetGlueTables = {
        Type = "Task"
        Resource = "arn:aws:states:::aws-sdk:glue:getTables"
        Parameters = {
          "DatabaseName.$" = "$.DatabaseName"
        }
        ResultPath = "$.GlueTablesResult"
        Next = "FilterIcebergTables"
      }
      FilterIcebergTables = {
        Type = "Pass"
        Parameters = {
          "TableNames.$": "$.GlueTablesResult.TableList[?(@.Parameters.table_type == 'ICEBERG')].Name",
          "DatabaseName.$": "$.DatabaseName"
        }
        ResultPath = "$.FilteredTables"
        OutputPath = "$.FilteredTables"
        Next = "CheckIfTablesExist"
      }
      CheckIfTablesExist = {
        Type = "Choice"
        Choices = [
          {
            Variable = "$.TableNames[0]"
            IsPresent = true
            Next = "MapVacuumTables"
          }
        ]
        Default = "NoTablesFound"
      }
      NoTablesFound = {
        Type = "Pass"
        Result = {
          Status = "Success"
          Message = "No Iceberg tables found to vacuum"
        }
        End = true
      }
      MapVacuumTables = {
        Type = "Map"
        ItemsPath = "$.TableNames"
        Parameters = {
          "TableName.$" = "$$.Map.Item",
          "DatabaseName.$" = "$.DatabaseName"
        }
        Iterator = {
          StartAt = "VacuumTable"
          States = {
            VacuumTable = {
              Type = "Task"
              Resource = "arn:aws:states:::athena:startQueryExecution.sync"
              Parameters = {
                "QueryString.$" = "States.Format('VACUUM {}.{};', $.DatabaseName, $.TableName.Value)",
                "WorkGroup" = "primary",
                "ResultConfiguration": {
                  "OutputLocation": "s3://${var.athena_queries_bucket_name}/${var.athena_queries_prefix}/"
                }
              }
              Retry = [
                {
                  ErrorEquals = ["Athena.TooManyRequestsException"],
                  IntervalSeconds = 5,
                  MaxAttempts = 10,
                  BackoffRate = 2
                },
                {
                  ErrorEquals = ["States.TaskFailed"], // ICEBERG_VACUUM_MORE_RUNS_NEEDED: Removed 20000 files in this round of vacuum, but there are more files remaining. Please run another VACUUM command to process the remaining files
                  IntervalSeconds = 1,
                  MaxAttempts = 10
                }
              ]
              Catch = [
                {
                  ErrorEquals = ["States.ALL"]
                  ResultPath = "$.Error"
                  Next = "HandleVacuumError"
                }
              ]
              Next = "VacuumSuccess"
            }
            VacuumSuccess = {
              Type = "Pass"
              Result = {
                Status = "Success"
                Message = "Vacuum query completed successfully"
              }
              End = true
            }
            HandleVacuumError = {
              Type = "Pass"
              Result = {
                Status = "Failed"
                Message = "Vacuum query failed"
              }
              End = true
            }
          }
        }
        Next = "WorkflowComplete"
      }
      WorkflowComplete = {
        Type = "Pass"
        Result = {
          Status = "Success"
          Message = "All vacuum operations completed"
        }
        End = true
      }
    }
  })

  logging_configuration {
    log_destination        = "${aws_cloudwatch_log_group.vacuum_iceberg_tables_logs.arn}:*"
    include_execution_data = true
    level                  = "ALL"
  }
}

# CloudWatch Log Group for Step Functions
resource "aws_cloudwatch_log_group" "vacuum_iceberg_tables_logs" {
  name              = "/aws/states/${var.stepfunction_name}"
  retention_in_days = 30
}